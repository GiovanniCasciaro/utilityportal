...
export default function NewContrattoForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || 'Errore durante la creazione del contratto')
      }
    } catch (err) {
  ...
  // Genera il numero del contratto
  useEffect(() => {
    if (!formData.numero) {
      const year = new Date().getFullYear()
      const random = Math.floor(Math.random() * 1000).toString().padStart(3, '0')
      setFormData(prev => ({ ...prev, numero: `CTR-${year}-${random}` }))
    }
  }, [])
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Nuovo Contratto</h1>
        <p className="text-gray-600">Crea un nuovo contratto TLC</p>
      </div>
